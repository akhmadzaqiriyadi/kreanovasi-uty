...
interface BookingHeroBannerProps {
  bannerRef?: React.Ref<HTMLElement>;
}

export function BookingHeroBanner({ bannerRef }: BookingHeroBannerProps) {
  const { header } = bookingConfig;

  return (
    <header
      ref={bannerRef}
      aria-label="Header Reservasi Ruangan UTY Creative Hub"
      className="relative w-full pt-24 pb-14 sm:pt-28 sm:pb-16 md:pt-32 md:pb-20 overflow-hidden"
    >
      {/* Background layers matching brand aesthetics */}
      <div
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
        ...
        >
          ...
          <ChevronRight
            ...
            aria-hidden="true"
          />
          <span className="text-primary-foreground/90">Reservasi Ruangan</span>
        </nav>
        ...
        <div className="max-w-3xl mx-auto text-center space-y-5 sm:space-y-6">
          ...
          <div className="space-y-1">
            ...
            <h1 className="text-3xl xs:text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-tight tracking-tight">
              <span className="text-secondary">{header.highlight}</span>
            </h1>
          </div>
